...
class ZoneValidationCookieHijackIT {
    ...
    void afterEach() {
        ...
        for (String zoneId : Arrays.asList("testzone1", "testzone2", "testzone3", "testzone4", "uaa")) {
            ScimGroup group = IntegrationTestUtils.getGroup(token, "", baseUrl, String.format("zones.%s.admin", zoneId));
            if (group != null) {
    ...
    void sessionCookieFromZone1DoesNotAllowZone2() {
        ...
        // Note: CORS policy configuration may not be available in this API version
        ...
        String originKey1 = SAML_ORIGIN + "-1";
        ...
        provider1.setOriginKey(originKey1);
        ...
        String originKey2 = SAML_ORIGIN + "-2";
        ...
        provider2.setOriginKey(originKey2);
        ...
        webDriver.get(String.format("%s/logout.do", baseUrl));
        webDriver.get(String.format("%s/logout.do", testZone1Url));
        webDriver.get(String.format("%s/logout.do", testZone2Url));

        // Login on zone 1 via SAML
        webDriver.get(String.format("%s/login", testZone1Url));
        
        // Navigate to SAML endpoint and perform SAML login
        webDriver.get(String.format("%s/saml2/authenticate/%s", testZone1Url, originKey1));
        ...
        // Should now be logged in to zone1, verify we're back at the zone URL
        Page.assertThatUrlEventuallySatisfies(webDriver,
            url -> url.contains(testZone1Url));
        ...
        webDriver.get(String.format("%s/login", testZone2Url));
